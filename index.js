...
function titelOnInput(e) {
  ...
  if (searchString == "")
    ...
    pr = d3
      .json(
        `https://en.wikipedia.org/w/api.php?action=opensearch&origin=*&limit=10&search=${searchString}`
      ) // `https://en.wikipedia.org/w/api.php?origin=*&action=opensearch&search=${searchString}&limit=10`
      .then((data) => data[1]);
